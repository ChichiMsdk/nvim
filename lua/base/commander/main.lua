...
local function on_line_selected(filepath, line)
  if line and string.sub(line, 1, 7) ~= "[FAV]->" then
    ...
    if string.find(filepath, "/.make") then
      vim.g.MakeLine = line
      return
    end
  end
...
function show_file_lines(filepath)
  ...
  local win_id, buf = create_window(nil, title)
  if lines then
    vim.api.nvim_buf_set_lines(buf, 0, -1, false, lines)
    highlight_fav_lines(buf)
  end
  vim.api.nvim_create_autocmd({"BufWriteCmd", "BufLeave", "BufWritePost"}, {
    buffer = buf,
    callback = function()
